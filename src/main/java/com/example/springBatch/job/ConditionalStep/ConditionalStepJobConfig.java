    ...
    public Job multipleJob(JobRepository jobRepository, PlatformTransactionManager platformTransactionManager){
       return  new JobBuilder("fileReadWriteJob",jobRepository)
                ...
                .start(multipleStepV1(jobRepository,platformTransactionManager))
                    .on("FAILED").to(multipleStepV2(jobRepository,platformTransactionManager)) //실패면 step2로
                .from(multipleStepV1(jobRepository,platformTransactionManager))
                    .on("COMPLETED").to(multipleStepV3(jobRepository,platformTransactionManager))//성공하면 step3으로
                 .from(multipleStepV1(jobRepository,platformTransactionManager))
                    .on("*").to(multipleStepV2(jobRepository,platformTransactionManager))//그외는 step2로
                 .end()
                .build();
    }
